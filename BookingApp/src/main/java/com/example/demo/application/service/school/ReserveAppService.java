...
public class ReserveAppService {
	...
	public Reservations buildReservation(FacilityReservationInfo facilityInfo, UserInfo userInfo) {
		...
		if (facilityInfo.getFacilityId() == 1) {
			reservation.setStartTime(LocalTime.of(15, 30));
			reservation.setEndTime(LocalTime.of(17, 30));
		} else {
			if (facilityInfo.getStartTime() == null || facilityInfo.getEndTime() == null) {
				throw new SystemException("開始時間または終了時間が設定されていません。");
			}
